		...
		$.ajax({
			...
			success: function (result) {
					if (typeof result.query == "undefined") {
						...
						var i;
						for (i = 0; i <= 9; i++) {
							$("#resultpost").append('<p class="posttitle"><a href="' + arr[i].fullurl + '" class="postlink" target="_blank">' + arr[i].title + '</a></p><p class="posturl">' + arr[i].fullurl + '</p><p class="postsnippet">' + arr[i].extract + '</p>'); //end append
						} //end of loop
						//console.log(result);
